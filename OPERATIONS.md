...
## 8p. Ninth stale-merchant triage (2026-09-15)

Same constraint as §8k-8o — no `DATABASE_URL` and no `curl`/`WebFetch` egress in this environment (`CONNECT tunnel failed, response 403` on every probed domain, confirmed again this pass); diagnosis leaned on CI log content plus DNS (`socket.gethostbyname_ex`, which does work).

Covered the last 4 completed scheduled runs (`34738650798` → `34878569812`, 2026-09-13 04:45 → 2026-09-14 18:04). All 4 were red at the top level; per-leg job status (not run conclusion) separated persistent breaks from cascade/noise, per the §8k methodology note. Also checked back through the intervening runs (`34562957686` → `34702443764`, 2026-09-11 → 2026-09-13) to establish exactly how long the persistent breaks have been running with zero green recoveries, since that's the bar §8o set for the two open items it left inconclusive.

| Merchant | Run(s) | Symptom | Verdict |
|---|---|---|---|
| **wc-smartdevices-ke** | 8/8 scheduled runs since #42 landed (2026-09-07) through the newest run checked (`34878569812`) — zero green runs in between | §8o's diagnosis holds exactly: every leaf logs a Cloudflare-issued 522/523 with an empty body, e.g. job `104092068445`: `[wc] smartdevices-ke/audio page1 zero cards (status 522, body head): '<html><head></head><body></body></html>'`. Playwright gets a real navigation response; Cloudflare itself is reporting the origin is unreachable. `playwright-stealth` is the strongest `client_type` this codebase has (`grep 'client_type ==' scrapers/common/*.py`). | **Deprecated** (PR #48). §8o's own bar — "still failing next pass with no green run in between" — is met: a full week, 8 consecutive runs, no recovery on any client. No client-side fix can route around an origin Cloudflare can't reach. 137 listings; catalog fully covered by Jumia / Kilimall / Hotpoint / Phone Place / solar specialists. |
| **wc-eamobitech-ke** | 8/8 scheduled runs since #43 landed (2026-09-07) through the newest run checked — zero green runs in between | Same story, different failure shape: every leaf raises `PlaywrightPoliteClient`'s own `RuntimeError` (`scrapers/common/base.py:153`/`:166`), surfacing in the diagnostic as `RetryError: RetryError[<Future ... raised RuntimeError>]` (job `104092070251`). That RuntimeError only fires when stealth Chromium gets a real response but Cloudflare is still serving a challenge page — the challenge itself is surviving stealth, not a network-layer failure. | **Deprecated** (PR #49). Same reasoning as smartdevices-ke: a full week with zero green runs past the strongest client this codebase has. Reads as an IP-reputation block on the GHA runner pool — the same root-cause class §8g already concluded for the Shopify batch. 52 listings; catalog fully covered by Jumia / Kilimall / Phone Place / camerastore-ke / Dixons. |
| **all-mybigorder** | 8/8 scheduled runs since 2026-09-11 (`34562957686` → `34878569812`) — new breakage, not flagged in §8o's window | Every category, every run, `RetryError` on page 1 (e.g. job `104092067174`: `mybigorder: mobile-phones-2nrkr page 1 failed (RetryError); skipping rest of this category`), tripping `ScraperYieldTooLow` at 236 listings on record. **Inconclusive** — `mybigorder.py`'s except-block only ever logged `e.__class__.__name__`, which for a tenacity `RetryError` is always the literal string `"RetryError"` regardless of cause. It never reached the wrapped exception, so nothing in 8 runs of logs says whether this is `HTTPStatusError` (TLS-fingerprint block → `cffi`, same playbook as eamobitech-ke/smartdevices-ke/solarshop-ke) or `Timeout`/`ConnectTimeout` (network-layer drop → deprecate, same playbook as zuka-ke/overtech-ke) — those two diagnoses call for opposite actions. One suggestive but non-conclusive data point: `mybigorder.com` now resolves to Cloudflare anycast IPs (`104.21.71.11`, `172.67.168.249`), contradicting the scraper's own docstring ("No Cloudflare wall") from 2026-07-07 — the site may have moved behind Cloudflare, which would point at the TLS-fingerprint diagnosis, but DNS alone can't rule out a network-layer block instead. | **Not fixed — diagnostics only** (PR #50). Flipping `client_type` on the DNS evidence alone would be exactly the kind of unverified guess the megatech-ke lesson (§8j) warns against — that revert happened because a plausible-sounding theory was acted on without re-verification. Instead, patched the except-block to print `str(e)` instead of `e.__class__.__name__`, matching the pattern `woocommerce.py`/`wc_store_api.py` already use (a gap this scraper never got in §8h/§8i's systemic fix). No behavior change. Next scheduled run's log will name the real wrapped exception; the fix (or deprecation) should follow from that, not from this pass's guess. |
| wc-hisense-kenya-ke, wc-quest-ke, solarstore-ke | No signal in this window | None of the merchants §8n/§8o flagged as "provisional noise" (quest-ke, hisense-kenya-ke) or "fixed, watch closely" (solarstore-ke, PR #46) recurred — solarstore-ke failed once in this window (`34768043407` only) with green runs immediately before and after, consistent with its playwright-stealth fix holding. | **Noise / holding.** No action. |
| phonesstore-ke, wc-nairobilaptops-ke | `34768043407` only | Fail-fast cascade from `wc-smartdevices-ke`/`solarstore-ke`/`wc-eamobitech-ke` failing earlier in the same matrix run (same mechanism §8k/§8m/§8o documented) — not real breaks. | **Noise** (cascade artifact). No action. |

**Cross-check against the deprecation list.** No already-deprecated merchant (7 Shopify, techonline-ke, zuka-ke, finetech-ke, overtech-ke, nairobitvshop-ke, tclke-ke, sollatek-ke's stray row, and now smartdevices-ke / eamobitech-ke from this pass) produced any signal in this window beyond the two new deprecations themselves.

**The §8o "sharper, more worrying data point" resolved as predicted, not averted.** §8o flagged that smartdevices-ke and eamobitech-ke had each stopped holding on playwright-stealth within one scheduled cycle of landing, and warned that if the pattern repeated with no recovery it would mean "GitHub Actions' IP pool is being blocked outright ... independent of client fingerprint" rather than something escalation could fix. Neither merchant recovered at any point across the full week checked this pass — the prediction held, and both are now deprecated rather than escalated again, because there is nothing left in this codebase's client ladder to escalate to. solarstore-ke's playwright-stealth fix (§8o, PR #46), by contrast, is holding cleanly (1 blip in 8 runs) — so this is not (yet) evidence that *every* Cloudflare-shielded Kenyan WooCommerce merchant is heading toward IP-reputation lockout, just that it happens to some of them, unpredictably, and playwright-stealth's fixes need a full week of clean runs before being trusted rather than declared victory after the first green run.
