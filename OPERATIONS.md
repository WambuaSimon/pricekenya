...
## 8p. Ninth stale-merchant triage (2026-09-13)

Same constraint as §8k-8o — no `DATABASE_URL` and no `curl`/`WebFetch` egress in this environment; diagnosis leaned on the `scrape.yml` matrix (`MIN_PRIOR_LISTINGS_FOR_CHECK` at 5 since §8j) plus CI log content and DNS.

Covered the last 4 completed scheduled runs (2026-09-11 04:38 → 2026-09-12 15:30, runs `34562957686` → `34702443764`). All 4 were red at the top level; per-leg job status separated persistent breaks from the one fail-fast cascade in the window (`phonesstore-ke` cancelled once in `34622080087`, same mechanism §8k/§8m/§8o documented — not a real phonesstore break).

**§8o's prediction landed exactly as feared.** solarstore-ke's playwright-stealth escalation (PR #46, merged 2026-09-11 16:21 UTC) did not survive a single scheduled run — it now shows the identical Cloudflare-origin-failure signature already stuck on smartdevices-ke and eamobitech-ke since §8n/§8o. All three of this codebase's Cloudflare-fronted merchants that reached the top of the escalation ladder (`playwright-stealth`) are now failing there with a server-side response (5xx or an unresolved challenge), not a client-fixable block:

| Merchant | Run(s) | Symptom | Verdict |
|---|---|---|---|
| **solarstore-ke** | 34622080087, 34673282131, 34702443764 — 3/3 since PR #46 landed; 34562957686 (the run just before) still failed on the pre-fix cffi client | Every run since the fix logs `[wc-store] solarstore-ke page1 HTTP 521, body head: '<html><head></head><body></body></html>'` — Cloudflare's "web server is down" origin error, empty body, no challenge markup. Same failure *class* as smartdevices-ke's 522/523 (§8o): a real HTTP response, Cloudflare itself reporting it can't reach the origin, not a network-layer drop and not a JS/TLS-fingerprint problem `client_type` can fix. | **Not fixed — inconclusive, no PR opened.** `playwright-stealth` is already the top of the ladder (confirmed again via `grep -rn 'client_type =='` on `scrapers/common/*.py` — nothing past it exists). Doesn't meet the dead-domain or network-layer-drop deprecation bar (DNS is fine, Cloudflare is issuing real HTTP responses) so not deprecated either. Left as-is. |
| **wc-smartdevices-ke** | 34562957686, 34622080087, 34673282131, 34702443764 — 4/4, no green run since PR #42 landed (2026-09-07) | Unchanged from §8o: every leaf still logs `page1 zero cards (status 522 or 523, body head): '<html><head></head><body></body></html>'`. Now six calendar days on the strongest available client with zero green runs. | **Not fixed — inconclusive, no PR.** Same reasoning as §8o; see cross-merchant note below. |
| **wc-eamobitech-ke** | 34562957686, 34622080087, 34673282131, 34702443764 — 4/4, no green run since PR #43 landed (2026-09-09) | Unchanged from §8o: `RetryError[RuntimeError]` on every leg — the stealth-patched Chromium gets a response but Cloudflare still serves a challenge page. | **Not fixed — inconclusive, no PR.** Same reasoning as §8o; see cross-merchant note below. |
| **all-mybigorder** | 34562957686, 34622080087, 34673282131, 34702443764 — 4/4; green through 34501761493 (2026-09-10 16:23) and 34438111965 (2026-09-10 04:40) | Every category logs `mybigorder: <slug> page 1 failed (RetryError); skipping rest of this category` — no status code, no body head, nothing else. `scrapers/merchants/mybigorder.py`'s except-block only ever printed `e.__class__.__name__`, which for a tenacity-wrapped failure is always the literal string `"RetryError"` — it never carried the wrapped exception, unlike `woocommerce.py`/`shopify.py`/`wc_store_api.py`, which already print `f"{type(exc).__name__}: {exc}"` (§8h/§8i) and get the underlying HTTP status or timeout class for free from `RetryError.__str__`'s embedded Future repr. This is a fresh break (first seen 2026-09-11 04:38, four calendar-clean runs before that), but its root cause — TLS fingerprint? Cloudflare-class block? plain 5xx? dead subcategory? — was genuinely undeterminable from the log as it stood. | **Diagnostics fixed, root cause still open.** Changed the print to `f"{type(e).__name__}: {e}"`, matching the established pattern. Ships in [PR — see below]; does not change scraper behavior, only what the next scheduled run's log will show. Root-cause fix deferred to whoever reads that log next — guessing at `client_type` here would be exactly the kind of blind flip this task's own brief warns against. |

**Cross-merchant note on smartdevices-ke / eamobitech-ke / solarstore-ke.** Three for three now: every merchant that reached `playwright-stealth` on this codebase's Cloudflare-fronted WooCommerce/WC-Store-API path has gone on to fail *at* Cloudflare rather than before it — 521/522/523 origin errors or an unresolved challenge, never a selector or markup problem. This is the trend §8o called "a sharper, more worrying data point," now with a third confirmation. None of the three meets this task's deprecation bar (dead domain, or a network-layer drop with no HTTP response at all) — Cloudflare is issuing real, if useless, HTTP responses in every case, and DNS resolves cleanly for all three domains. But there is also nothing left in `scrapers/common/woocommerce.py` or `wc_store_api.py` to escalate to, and the combined catalog at stake is non-trivial (137 + 52 + 46 = 235 listings). This reads as a policy decision (accept the loss and deprecate all three as a group, or invest in a residential-proxy client per merchant) rather than a per-merchant diagnosis this task is scoped to make — flagging for a human call rather than acting unilaterally, per the task's own instruction not to deprecate on anything short of the two stated criteria.
