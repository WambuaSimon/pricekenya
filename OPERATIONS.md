...
## 8p. Ninth stale-merchant triage (2026-09-21)

Same constraint as §8k-8o — no `DATABASE_URL` in this environment, signal came entirely from the `scrape.yml` matrix (`MIN_PRIOR_LISTINGS_FOR_CHECK` at 5 since §8j). `gh` CLI was also unavailable this pass; used the GitHub MCP tools (`actions_list`/`get_job_logs`) instead — same data, different transport.

Covered the last 4 scheduled runs (2026-09-19 04:35 → 2026-09-20 16:04, runs `35421722659` → `35521557261`), all 4 red at the top level. Per-leg job status separated persistent breaks from cascade/noise, per the §8k methodology note. Also sampled 3 runs further back (`34562957686` 09-11 04:38, `34807724512` 09-14 04:54, `35056989837` 09-16 04:47) specifically to check whether the two merchants §8o flagged as "genuine deprecation candidate if it persists with no green run" had in fact had one.

| Merchant | Run(s) | Symptom | Verdict |
|---|---|---|---|
| **wc-smartdevices-ke** | 4/4 in the primary window, and every one of the 3 sampled runs back to 2026-09-11 (right after §8o) — no green run anywhere in that 10-day, ~20-run span | Same Cloudflare 522/523 origin-error signature §8o already diagnosed (`page1 zero cards (status 522, body head): '<html><head></head><body></body></html>'`), unchanged, on `playwright-stealth` — already the top of this codebase's client escalation ladder. | **Deprecated** (PR #55). §8o's own bar ("if it's still failing next pass with no green run in between, this becomes a genuine deprecation candidate") is met — 10 days, not one pass. Same root-cause class as the 7 Shopify stores (§8g) and techonline-ke (§8h): IP-reputation block on the GHA runner pool, residential-proxy-only, not a client-side fix. Removed from `wc_merchants.py` and the workflow matrix + Chromium gate. 137 listings; catalog (audio/cooking/inverters/solar/laptops/phones/tvs/appliances) already covered by Jumia/Kilimall/Hotpoint/Phone Place. |
| **wc-eamobitech-ke** | 4/4 in the primary window, and every one of the 3 sampled runs back to 2026-09-11 — same no-green-run span as smartdevices-ke | Same "Cloudflare challenge unresolved" `RuntimeError` §8o already diagnosed, unchanged, on `playwright-stealth`. | **Deprecated** (PR #56). Same reasoning as smartdevices-ke — §8o's own bar met, same root-cause class as §8g/§8h. 52 listings; catalog (audio/cameras/laptops/phones/tablets/tvs) already covered by Jumia/Kilimall/Phone Place/Camera Store. |
| **audiocom-ke** | 4/4 in the primary window; confirmed green as recently as `35056989837` (2026-09-16 04:47) | New escalation, not a standing issue. `[wc-store] audiocom-ke page1 GET failed: RetryError: RetryError[<Future ... raised HTTPError>]` on curl_cffi (the WC Store API scraper's default `client_type`) — curl_cffi's own HTTP error status, not a `Timeout`. Same signature class that already forced smartdevices-ke/eamobitech-ke (§8n) and solarstore-ke (§8o) past cffi. | **Fixed** (PR #54). `client_type="playwright-stealth"` in `scrapers/merchants/audiocom.py` + added `audiocom-ke` to the Chromium install gate. |
| wc-le-ke | `35521557261` (newest) only | `[wc] le-ke/audio page1 GET failed: RetryError: RetryError[<Future ... raised HTTPStatusError>]` on plain httpx (`client_type` unset) — the TLS-fingerprint-discrimination signature that's usually a one-line `cffi` fix. But green on the 3 runs immediately before it. | **Noise, provisionally** — one run only, per the megatech-ke lesson in §8j. Flagging because the signature is a clean, known-fixable match rather than generic flakiness; if it recurs next pass the fix is `client_type: "cffi"`, no further diagnosis needed. |
| wc-quest-ke | `35421722659` (oldest of the 4) only | Zero cards, `client_type` unset. | **Noise.** Green on the 3 runs after it. Recovered on its own — same as its single appearance in §8n, which also didn't recur in §8o. |
| wc-hisense-kenya-ke | `35490237437` only | Cloudflare Turnstile, same as always. | **Noise.** Green on the runs immediately before and after. |
| wc-housewife-ke | `35521557261` only, `cancelled` not `failure` | Fail-fast cascade: `wc-le-ke`/`wc-smartdevices-ke`/`audiocom-ke`/`wc-eamobitech-ke` failing earlier in the same matrix run cancelled it mid-queue, per the §8k mechanism. | **Noise** (cascade artifact). No action. |

**Cross-check against the deprecation list.** No already-deprecated merchant (7 Shopify, techonline-ke, zuka-ke, finetech-ke, overtech-ke, nairobitvshop-ke, tclke-ke, sollatek-ke's stray row) produced any signal in this window. smartdevices-ke and eamobitech-ke join it as of this pass.

**Coverage-test upkeep.** `tests/test_scrape_coverage.py::test_woocommerce_merchant_is_active` used `smartdevices-ke` as its example of an ACTIVE WC merchant — switched to `fivestar-ke` (still in `WC_MERCHANTS`) alongside the smartdevices-ke deprecation PR, and added `classify("smartdevices-ke") is Coverage.DEPRECATED` to `test_deprecation_is_derived_not_hardcoded` so the derived-not-hardcoded guarantee covers this pass's removal too.
