...
## 8s. Not a merchant outage: unpinned sqlmodel broke every DB write (2026-09-22/23)

Numbered §8s, not §8p, because three open-but-unmerged PRs from prior passes of this same recurring task already claim §8p (#51), §8q (#53) and §8r (#54) for their own write-ups against the `main` tip this pass also started from — see the process-gap note below. Whichever of those merges first, the others (including this one) should be renumbered to stay chronological.

`scrape.yml`'s last 10 scheduled runs (2026-09-18 16:22 → 2026-09-23 04:43) were **all** red at the top level, and 3 of the last 4 failed on nearly the *entire* matrix — 60+ of ~65 legs, including merchants with zero outage history (`all-audio` / Jumia+Kilimall, `hotpoint-*`, `all-naivas`, `all-carrefour`). That blast radius doesn't fit any per-merchant bot-mitigation signature this log has ever recorded, so this pass didn't diagnose it as one.

**Root cause.** `pyproject.toml` pins `sqlmodel>=0.0.22` with no ceiling. CI (and this environment's own `pip install -e .`) resolves `sqlmodel==0.0.46`, whose `UTCDateTime` column type (`sqlmodel/sql/sqltypes.py`) now raises at write time instead of silently accepting a naive `datetime`:

```
ValueError: Datetime values must have timezone information. Use datetime.now(timezone.utc), or annotate the field with NaiveDatetime for naive storage.
[SQL: UPDATE listing SET last_checked_at=%(last_checked_at)s::TIMESTAMP WITH TIME ZONE WHERE listing.id = %(listing_id)s::INTEGER]
```

(quoted verbatim from job `107048234444`, run `35819499295`, leg `scrape (all-audio)` — 48s to failure, no scraping even attempted, the crash is in `_upsert_one_listing`'s `session.commit()`)

Every `datetime` field in `db/models.py` used `default_factory=datetime.utcnow` (naive) against a Postgres `TIMESTAMP WITH TIME ZONE` column — this worked under every previously-resolved sqlmodel version, and stopped working the moment CI re-resolved dependencies against a release that tightened the check. `scrapers/ingest.py`'s `_upsert_one_listing` writes `Listing.last_checked_at` and `PriceHistory.observed_at` on every single successful fetch, so once a scraper got past its own bot-mitigation and had real listings to write, the ingest step itself crashed — which is why merchants with *no* bot-mitigation history (Jumia, Hotpoint, Naivas…) failed identically to the ones already fighting Cloudflare. `alerts/dispatcher.py`'s `Alert.last_notified_at` write hit the same wall in the `alerts` job.

**Confirmed via the `health` job's own coverage table** (job `107062001166`, run `35819499295`): 39 of 55 merchants read `BROKEN (active & stale > 30h)`, with `last_checked_at` timestamps for the bulk of the active roster clustering at 2026-09-21 18:57–19:58 UTC — i.e. every merchant's *last successful write*, not 39 independent breakages. That matches run `35636855790` (2026-09-21 18:12), the last scheduled run before this pass's window, which was clean except for the 3 already-known legs (see the PR-backlog note below). The regression window is therefore ~2026-09-21 19:58 UTC → ~2026-09-22 04:53 UTC, the gap between that run and the next scheduled one — consistent with an upstream PyPI release landing in that window, not a repo change (no commit touched `db/models.py` or `pyproject.toml` in between).

**Also broke the full local test suite**, independent of CI: a fresh `pip install -e .` in this environment resolves the same `sqlmodel==0.0.46`, and `python -m pytest -q` before any fix went from 551 collected tests to **551 errors** — nearly every test that touches the DB layer fails at fixture setup (`tests/conftest.py`'s session-scoped `_create_default_engine_tables` inserts fixture rows through the same `default_factory=datetime.utcnow` path). This means `ci.yml` would have started failing on every future PR too, not just the scheduled scrape — the last green `ci.yml` run (`35568694166`, 2026-09-21 06:29, PR #54) predates the regression window above.

**Fix.** Not a version pin (that just delays the next unpinned-dependency surprise) — the error message names the actual correct fix, and the codebase already had the pattern in several files (`app/routes/admin.py`'s `now_utc`, `scripts/scrape_health.py`, `app/templating.py`). Replaced every `datetime.utcnow()` / `default_factory=datetime.utcnow` that ends up in a DB write with `datetime.now(UTC)` (`db/models.py` gains a small `_utcnow()` helper used by all 11 `default_factory` fields), across `db/models.py`, `scrapers/ingest.py`, `alerts/dispatcher.py`, `app/routes/{admin,reviews,products,pages,meta,internal}.py`, `scripts/{prune_merge_candidates,rebuild_sitemap}.py`, `seed/load.py`, `matching/llm_extract.py`, and the 9 test files that constructed model rows the same naive way. Several call sites (`app/routes/admin.py`'s click-dashboard cutoffs, `scripts/rebuild_sitemap.py`, `matching/llm_extract.py`'s daily-cap cutoff, 3 test fixture files) had been deliberately *stripping* tzinfo with `.replace(tzinfo=None)` to match the old naive columns — those strips are removed rather than worked around, since the columns are aware now and stripping would just reintroduce the same `ValueError` on the next write. No Postgres migration needed: the live column type was already `TIMESTAMP WITH TIME ZONE` (visible in the SQL quoted above) — this was purely a Python-side write-path bug, not a schema mismatch, so existing rows are unaffected.

`python -m pytest -q`: 551 passed, 1 skipped (was 551 errors). `python -m ruff check` clean on every touched directory.

**Process gap, independently re-confirmed.** This pass also found what §8r (PR #54, unmerged) already flagged: 7 open PRs (#47, #48, #49, #50, #51, #53, #54) from the last several scheduled runs of this recurring task are sitting unmerged against `main`, all CI-green. Among them, #48 and #49 already deprecate `smartdevices-ke` and `eamobitech-ke` with solid evidence (Cloudflare 522/523 and unresolved-challenge signatures that §8o and §8r both independently reconfirmed persistent), and #54 fixes `audiocom-ke`. Per this task's own instruction not to re-diagnose a merchant already covered, **this pass did not re-open PRs for any of the three** — the last clean signal before the systemic bug (run `35636855790`, 2026-09-21 18:12) shows exactly those 3 legs failing and nothing else new. Merging the existing backlog (in the order #47/#50 → #48 → #49 → #54, then the documentation-only #51/#53) remains the single highest-leverage action against the "39 BROKEN merchants" figure, independent of this PR.

**Not re-verified this pass:** whether the individual merchant-level diagnoses from §8n/§8o (smartdevices-ke, eamobitech-ke Cloudflare-origin/challenge signatures) still hold once this fix is merged and the matrix is scraping again — the entire CI signal for the last ~1.5 days was this regression, not merchant behavior, so there is nothing fresh to re-diagnose them against yet. Next pass should re-check both once the systemic fix and the backlog PRs have landed.
